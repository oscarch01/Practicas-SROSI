...
# Descripcion: Crear usuario mediante determinadas reglas de forma interactiva con quotas y permisos sudo
###################################################################
import os
import crypt
import re
...
add_sudoers = raw_input("Desea dar permisos de sudo al usuario? [S/N]: ")

if add_sudoers == "S":
    commands = ""
    add_other = True
    while add_other:
        command = raw_input("Que comando desea que el usuario pueda ejecutar? (Escriba ALL si desea que pueda ejecutar todos): ")
        if command == "ALL":
            commands = command
            add_other = False
        else:
            q_a_other = raw_input("Desea aniadir otro comando? [S/N]: ")
            if q_a_other == "S":
    ...
    os.system("echo '"+user+" ALL=(ALL:ALL) "+commands+"' | sudo EDITOR='tee -a' visudo")
